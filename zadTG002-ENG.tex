\begin{task}
\TT{Oblicz transformatę Fouriera sygnału $f(t)=A \cdot \mathbb{1}(t) \cdot e^{-a\cdot t}$ przedstawionego na rysunku oraz narysuj jego widmo amplitudowe i fazowe.}{Compute the Fourier transform of a impulse $f(t)=A \cdot \mathbb{1}(t) \cdot e^{-a\cdot t}$ shown below. Compute and draw magnitude and phase spectra.}
...
\TT{Transformata sygnału $f(t)$ to $F(\jmath \omega)=\frac{A}{ a + \jmath \cdot \omega}$.}{The Fourier transform of the $f(t)$ signal is equal to $F(\jmath \omega)=\frac{A}{ a + \jmath \cdot \omega}$.}
