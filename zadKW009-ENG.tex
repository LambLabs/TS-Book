...
\TT{Podstawiamy do wzoru wzór naszej funkcji w pierwszym okresie $k=0$}{For the period $t \in \left(\frac{-T}{4};\frac{3 \cdot T}{4}\right)$ we get:}
...
\TT{Wartość współczynnika $b_k$ wynosi $\frac{2 \cdot A}{k \cdot \pi} \cdot \left( 1 - cos\left(k \cdot \frac{\pi}{2} \right) \right)$}{The $b_k$ coefficients equal to $\frac{2 \cdot A}{k \cdot \pi} \cdot \left( 1 - cos\left(k \cdot \frac{\pi}{2} \right) \right)$.}
...
f(t) &= \sum_{k=1}^{\infty} \left[\frac{2 \cdot A}{k \cdot \pi} \cdot \left( 1 - cos\left(k \cdot \frac{\pi}{2} \right) \right) \cdot sin\left(k \cdot \frac{2\pi}{T} \cdot t\right)\right]
\end{align*}

\TT{W przypadku sumowania do $k_{max}=1$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=1$ will results in:}
...
\TT{W przypadku sumowania do $k_{max}=2$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=2$ will results in:}
...
\TT{W przypadku sumowania do $k_{max}=3$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=3$ will results in:}
...
\TT{W przypadku sumowania do $k_{max}=5$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=5$ will results in:}
...
\TT{W przypadku sumowania do $k_{max}=6$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=6$ will results in:}
...
\TT{W przypadku sumowania do $k_{max}=11$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=11$ will results in:}
...
\TT{W przypadku sumowania do $k_{max}=21$ otrzymujemy}{A partial approximation of the $f(t)$ signal for $k_{max}=21$ will results in:}
...
\TT{W granicy sumowania do $k_{max}=\infty$ otrzymujemy oryginalny sygnał.}{Approximation of the $f(t)$ signal for $k_{max}=\infty$ will results in oryginal signal.}
