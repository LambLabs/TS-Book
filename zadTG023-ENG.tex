...
\TT{Inne możliwości opisu sygnał $f(t)$ za pomocą sygnałów elementarnych:}{Other ways to describe the $f(t)$ signal using elementary signals:}
...
\TT{\textbf{SPOSÓB II:}}{\textbf{SOLUTION II:}}

\TT{Rozważmy następujący opis sygnału $f(t)$ za pomocą sygnałów elementarnych:}{Let's consider the following description of the $f(t)$ signal:}
...
\TT{Transformata sygnału $f_{1}(t)$ to:}{The Fourier transform of the $f_{1}(t)$ signal is equal to:}

\begin{equation}
F_{1}(\jmath \omega) = \mathcal F\{f_{1}(t)\} = 4 \cdot Sa\left(2 \cdot \omega\right)
\end{equation}

\TT{Wyznaczmy transformtę sygnału $f_{2}(t)$, czyli $F_{2}(\jmath \omega)$.}{Calculate the Fourier transform $F_{2}(\jmath \omega)$ for the second signal $f_{2}(t)$.}

\TT{Z treści zadania wiemy, że:}{We know that:}
$\mathcal F \{\Pi(t)\} = Sa\left(\frac{\omega}{2}\right)$.

\TT{Wykorzystując twierdzenie o przesunięciu w dziedzinie czasu:}{Based on the shifting in time theorem:}
...
\TT{Transformata sygnału $f_{2}(t)$ to:}{The Fourier transform of the $f_{2}(t)$ signal is equal to:}

\begin{equation}
F_{2}(\jmath \omega) = \mathcal F\{f_{2}(t)\} = Sa\left(\frac{\omega}{2}\right) \cdot e^{-\jmath \cdot \omega \cdot \frac{3}{2}}
\end{equation}

\TT{Wyznaczmy transformtę sygnału $f_{3}(t)$, czyli $F_{3}(\jmath \omega)$.}{Calculate the Fourier transform $F_{3}(\jmath \omega)$ for the third signal $f_{3}(t)$.}
...
\TT{Transformata sygnału $f_{3}(t)$ to:}{The Fourier transform of the $f_{3}(t)$ signal is equal to:}

\begin{equation}
F_{3}(\jmath \omega) = \mathcal F\{f_{3}(t)\} = Sa\left(\frac{\omega}{2}\right) \cdot e^{\jmath \cdot \omega \cdot \frac{3}{2}}
\end{equation}

\TT{Czyli transformata sygnału $f(t)$ to:}{Finally, the Fourier transform of the $f(t)$ signal is equal to:}
...
\TT{Transformata sygnału $f(t) = \Pi\left(\frac{t}{4}\right) + \Pi\left(t - \frac{3}{2}\right) + \Pi\left(t + \frac{3}{2}\right)$ to $F(\jmath \omega)=4 \cdot Sa\left(2 \cdot \omega\right) + 2 \cdot Sa\left(\frac{\omega}{2}\right) \cdot cos\left(\omega \cdot \frac{3}{2}\right)$.}{The Fourier transform of the $f(t) = \Pi\left(\frac{t}{4}\right) + \Pi\left(t - \frac{3}{2}\right) + \Pi\left(t + \frac{3}{2}\right)$ signal is equal to $F(\jmath \omega)=4 \cdot Sa\left(2 \cdot \omega\right) + 2 \cdot Sa\left(\frac{\omega}{2}\right) \cdot cos\left(\omega \cdot \frac{3}{2}\right)$.}

\TT{\textbf{Uwagi końcowe:}}{\textbf{Final remarks:}}

\TT{Rozwiązania uzyskane sposobem I i II to odpowiednio:}{Results derived for Solution I and II are equal to:}

\begin{align*}
F(\jmath \omega)&=8 \cdot Sa \left(2 \cdot \omega\right) - 2 \cdot Sa\left(\omega\right)\\
F(\jmath \omega)&=4 \cdot Sa\left(2 \cdot \omega\right) + 2 \cdot Sa\left(\frac{\omega}{2}\right) \cdot cos\left(\omega \cdot \frac{3}{2}\right)
\end{align*}

\TT{Można wykazać, że jest to ta sama transformata (mimo, iż uzyskane równania opisujące transformatę są różne).}{It can be shown that this is the same transform (although the obtained equations describing the transform are different).}
