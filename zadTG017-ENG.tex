\begin{task}
\TT{Przedstaw poniższe sygnały jako funkcję liniową sinusów i kosinusów wykorzystując wzory Eulera.}{Expand the following signals into a sum of sine and cosine functions, and a constant, by using the Euler identities.}
...
\TT{Wzory Eulera}{Euler identities}:
\begin{align*}
...
\TT{Współczynniki dwumianu Newtona $\binom{n}{k}$ są $k-tym$ elementem w $n-tym$ wierszu trójkąta Pascala (liczymy od $0$). Każdy element trójkąta jest sumą dwóch elementów nad nim. Poniżej przedstawiono przykład dla $n=6$:}{The binomial coefficient $\binom{n}{k}$ appears as the $kth$ entry in the $nth$ row of Pascal's triangle (counting starts at 0). Each entry is the sum of the two above it. Below, example for $n=6$ is presented:}
