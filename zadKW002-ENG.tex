...
\TT{Oblicz wartość średnią sygnału $f(t)=\mathbf{1}(t)\cdot e^{-a\cdot t}\cdot sin\left(\frac{2\pi}{T}\cdot t \right )$ przedstawionego na rysunku :}{Calculate the mean value of the signal $f(t)=\mathbf{1}(t)\cdot e^{-a\cdot t}\cdot sin \left( \frac{2\pi}{T} \cdot t \right)$ given below:}
...
\TT{Wartość średnią sygnału wyznaczamy ze wzoru:}{The mean value for non-periodic signals is defined by:}

\begin{equation}
\bar{f}=\lim_{\tau \rightarrow \infty }\frac{1}{\tau}\int_{-\frac{\tau}{2}}^{\frac{\tau}{2}}f(t) \cdot dt
\end{equation}

\TT{Podstawiamy do wzoru na wartość średnią wzór naszej funkcji:}{Compute mean value:}
...
\TT{Średnia wartość sygnału wynosi $0$.}{The mean value equals to $0$.}
\end{task}
